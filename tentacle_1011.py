...
def tentacle(input_data):
    ...
    try:
        ...
        if ',' in input_data:
            ...
            # Count vowels in the reversed items
            vowels = sum(1 for char in reversed_items if char.lower() in 'aeiou')
            return f"{reversed_items}, vowel count: {vowels}"
        elif input_data.isdigit():
            # If it's a single number, return its square, cube, factorial, and square root
            number = int(input_data)
            factorial = math.factorial(number)
            return f"square: {number ** 2}, cube: {number ** 3}, factorial: {factorial}, square root: {math.sqrt(number):.2f}"
