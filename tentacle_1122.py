...
def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in input_data.lower():
            ...
            # For generic HTML documents, extract and return the title and relevant classes if present
            title_match = re.search(r'<title>(.*?)</title>', input_data, re.IGNORECASE)
            class_match = re.findall(r'class="([^"]*)"', input_data, re.IGNORECASE)
            
            result = 'generic html document detected'
            if title_match:
                result += f": {title_match.group(1).strip().lower()}"
            if class_match:
                classes = ','.join(sorted(set(cls.lower() for cls in class_match)))
