    ...
    try:
        ...
        if isinstance(input_data, str):
            if ',' in input_data:
                ...
                return ' '.join(input_data.strip().lower().split())
        elif isinstance(input_data, (list, dict)):
            try:
                # Attempt to convert the input to a JSON string
                json_string = json.dumps(input_data)
                # Sort the keys in the JSON string
                sorted_json = json.loads(json_string)
                sorted_json_string = json.dumps(sorted_json, sort_keys=True)
                return sorted_json_string.lower()
            except:
                # If JSON conversion fails, convert to string and process as above
                return ' '.join(str(input_data).strip().lower().split())
        else:
            # For other data types, convert to string and process as above
            return ' '.join(str(input_data).strip().lower().split())