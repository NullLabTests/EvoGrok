def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lowercase_input:
            ...
            return f"data analysis html document detected, length: {len(sorted_content)}, title: Data Analysis, content: {sorted_content}"
        elif 'mathematics' in lowercase_input:
            content = ' '.join(lowercase_input.split()[2:])  # Remove DOCTYPE and first tag
            sorted_content = ','.join(sorted(content.split()))
            return f"mathematics html document detected, length: {len(sorted_content)}, title: Mathematics, content: {sorted_content}"
        elif 'text processing' in lowercase_input:
            content = ' '.join(lowercase_input.split()[2:])  # Remove DOCTYPE and first tag
            sorted_content = ','.join(sorted(content.split()))
            return f"text processing html document detected, length: {len(sorted_content)}, title: Text Processing, content: {sorted_content}"
        else:
    ...
    try:
        ...
        return f"{sorted_result}, processed length: {len(sorted_result)}, original result: {result}, result length: {result_length}, original input length: {original_length}, original input: {input_data}"
    except:
        ...
        return f"{sorted_input}, processed length: {processed_length}, original input length: {original_length}, original input: {input_data}"