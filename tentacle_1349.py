...
def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        # Determine the type of HTML document based on known patterns and keywords
        if 'data analysis' in lowercase_input or 'data analysis' in get_html_title(lowercase_input):
            return "data analysis html document detected"
        elif 'mathematics' in lowercase_input or 'mathematics' in get_html_title(lowercase_input):
            return "mathematics html document detected"
        elif 'text processing' in lowercase_input or 'text processing' in get_html_title(lowercase_input):
            return "text processing html document detected"
        else:
            # Attempt to extract the page title from the HTML
            title = get_html_title(lowercase_input)
            if title:
...
def get_html_title(html_content):
    # Helper function to extract the HTML title
