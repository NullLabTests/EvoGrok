    ...
    try:
        ...
        if ',' in input_str:
            ...
            if 'data analysis' in input_str:
                ...
                if '<html' in input_str or '<body' in input_str or '<head' in input_str:
                    return "potential html fragment detected"
                else:
                    # If no HTML fragments, check for common file extensions
                    if input_str.endswith(('.txt', '.csv', '.json', '.xml')):
                        return f"file with {input_str[-4:]} extension detected"
                    else:
