...
def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (list, tuple)):
            # Calculate the average of numeric values and count non-numeric values
            ...
            if numeric_values:
                ...
                numeric_output = f"average of {','.join(map(str, numeric_values))}: {average:.2f}"
                
                if non_numeric_values:
                    return f"{numeric_output}; non-numeric values: {','.join(map(str, non_numeric_values))}"
                else:
