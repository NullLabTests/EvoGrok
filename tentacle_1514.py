def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lowercase_input:
            ...
            elif 'literature' in lowercase_input:
        ...
        if isinstance(input_data, str):
            ...
            if ',' in input_data:
                ...
                if all(val.replace('.', '').replace('-', '').isdigit() for val in result.split(',')):
                    # If it's a list of numbers, calculate and return their average, median, and range
                    ...
                    return f"statistics of {result}: average={average:.2f}, median={median:.2f}, range={range_val:.2f}"
                else:
                    return result
            else:
