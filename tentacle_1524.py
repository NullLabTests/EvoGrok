def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lowercase_input:
            ...
            # For generic HTML, attempt to extract and sort unique words
            words = sorted(set(lowercase_input.split()))
            # Remove common HTML tags and attributes
            filtered_words = [word for word in words if not word.startswith(('<', '>')) and '=' not in word]
            return f"generic html document detected with keywords: {' '.join(filtered_words)}"
    ...
    try:
        ...
        if isinstance(input_data, str):
            ...
            if ',' in cleaned_input:
                ...
                cleaned_values = sorted(set(value.strip() for value in sorted_values if value.strip()))
