        ...
        if 'data analysis' in result:
            ...
            try:
                ...
                if result.strip().lower().startswith('<html') or result.strip().lower().startswith('<!doctype'):
                    ...
                    # New feature: Check for common text processing patterns
                    if any(keyword in result.lower() for keyword in ['tokenization', 'stemming', 'lemmatization', 'stop words']):
                        return f"{result}, text processing pattern detected"
                    else:
                        return result