def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith(('<!doctype', '<html')):
    ...
    try:
        ...
        return f"mathematical expression detected: {str(result).lower()}"
    except:
        ...
        if 'data analysis' in result:
            ...
            if result.strip().lower().startswith(('<html', '<!doctype')):
                ...
                try:
                    ...
                    # If no special conditions are met, return the processed input
                    return result