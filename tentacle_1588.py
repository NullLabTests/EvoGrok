def tentacle(input_data):
    ...
    # Additional processing for numeric results
    if output.replace('.', '').isdigit():
        # If the output is a number, round it to 2 decimal places
        rounded_output = round(float(output), 2)
        # Check if the rounded number is an integer
        if rounded_output.is_integer():
            output = str(int(rounded_output))
        else:
            output = str(rounded_output)
    
    # Check for specific keywords and add context
    keywords = {
        'data': 'related to data analysis',
        'math': 'related to mathematics',
        'text': 'related to text processing'
    }
