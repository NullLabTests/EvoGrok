        ...
        if 'data analysis' in result:
            ...
            try:
                ...
                if '@' in result and '.' in result.split('@')[-1]:
                    return f"Potential email address detected: {result}"
                
                # New feature: Check for potential URLs
                if result.startswith(('http://', 'https://', 'www.')):
                    return f"Potential URL detected: {result}"
                
                # If no special cases match, return the result
                return result