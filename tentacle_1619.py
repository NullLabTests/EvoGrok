def tentacle(input_data):
    ...
    # New feature: Handle list inputs
    if isinstance(input_data, list):
        # Convert list to string, process it, and return as a list
        processed_list = tentacle(','.join(map(str, input_data))).split(',')
        return [item.strip() for item in processed_list]

    # New feature: Handle dictionary inputs
    if isinstance(input_data, dict):
        # Process each key-value pair and return as a new dictionary
        return {tentacle(key): tentacle(value) for key, value in input_data.items()}
