def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # For dictionaries, sort keys, remove duplicate values, and join
            sorted_keys = sorted(result.keys())
            unique_values = sorted(set(str(value).lower() for value in result.values()))
            return f"keys: {','.join(map(str, sorted_keys))}, unique values: {','.join(unique_values)}"
        else:
        ...
        if isinstance(input_data, str):
            ...
            unique_values = sorted(set(str(value).lower() for value in input_data.values()))
            return f"keys: {','.join(map(str, sorted_keys))}, unique values: {','.join(unique_values)}"
        else:
