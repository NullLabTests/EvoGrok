def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lower_input:
            ...
            if 'class="mw-page-title-main"' in lower_input:
                ...
                if 'class="vector-body"' in lower_input:
                    return "wikipedia article html document detected"
                elif 'class="mediawiki"' in lower_input:
                    return "wikipedia main page html document detected"
                else:
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # the original result, and a simple mathematical operation
            sorted_digits = ''.join(sorted(str(result).lower()))
            sum_of_digits = sum(int(digit) for digit in str(abs(result)) if digit.isdigit())
            simple_operation = result * 2 if isinstance(result, int) else result ** 2
            return f"{sorted_digits}, sum: {sum_of_digits}, original: {result}, simple operation: {simple_operation}"
