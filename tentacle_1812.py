def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # For strings, split, sort, remove duplicates, join, and calculate word count
            ...
            return f"{joined_items}, word count: {word_count}"
        elif isinstance(result, (list, tuple, set)):
            # For collections, sort elements, remove duplicates, join, and calculate item count
            sorted_items = sorted(set(str(item).lower() for item in result))
            joined_items = ','.join(sorted_items)
            item_count = len(sorted_items)
            return f"{joined_items}, item count: {item_count}"
        elif isinstance(result, dict):
            # For dictionaries, sort keys, join key-value pairs, and calculate entry count
            sorted_items = sorted((f"{k}:{v}" for k, v in result.items()))
            joined_items = ','.join(sorted_items)
            entry_count = len(sorted_items)
            return f"{joined_items}, entry count: {entry_count}"
        else:
        ...
        if isinstance(input_data, str):
            ...
            # Split the input, sort it, remove duplicates, join it back together, and calculate word count
            sorted_items = sorted(set(item.strip().lower() for item in input_data.split()))
            joined_items = ','.join(sorted_items)
            word_count = len(sorted_items)
            return f"{joined_items}, word count: {word_count}"
        elif isinstance(input_data, (list, tuple, set)):
            # If it's a collection, sort its elements, remove duplicates, join them, and calculate item count
