def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            # For numbers, return sorted digits, sum, product, and the original result
            sorted_digits = ''.join(sorted(str(abs(result))))
            ...
            return f"Number: {sorted_digits}, sum: {sum_of_digits}, product: {product_of_digits}, original: {result}"
            ...
            return f"Words: {','.join(sorted_items)}, count: {word_count}"
            ...
            return f"Items: {','.join(sorted_items)}, count: {item_count}"
        elif isinstance(result, dict):
            # For dictionaries, sort keys, join, and count key-value pairs
            sorted_keys = sorted(result.keys())
            key_value_count = len(sorted_keys)
            return f"Keys: {','.join(map(str, sorted_keys))}, count: {key_value_count}"
        else:
            # For other types, return a lowercase string representation, its length, and the original result
            return f"Other: {str(result).lower()}, length: {len(str(result))}, original: {result}"
        ...
        if isinstance(input_data, str):
            ...
            item_count = len(sorted_items)
            return f"Text: {','.join(sorted_items)}, count: {item_count}"
            ...
            item_count = len(sorted_items)
            return f"Collection: {','.join(sorted_items)}, count: {item_count}"
        elif isinstance(input_data, dict):
            # If it's a dictionary, sort keys, join them, and count key-value pairs
            sorted_keys = sorted(input_data.keys())
            key_value_count = len(sorted_keys)
            return f"Dictionary: {','.join(map(str, sorted_keys))}, count: {key_value_count}"
        else:
            # For other types, return a lowercase string representation, its length, and the original input
            return f"Other: {str(input_data).lower()}, length: {len(str(input_data))}, original: {input_data}"