def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # the original result, and whether it's prime
            sorted_digits = ''.join(sorted(str(result).lower()))
            sum_of_digits = sum(int(digit) for digit in str(abs(result)) if digit.isdigit())
            is_prime = is_prime_number(abs(result))
            return f"{sorted_digits}, sum: {sum_of_digits}, prime: {is_prime}, original: {result}"
            ...
            # its length, type, and the original result
        ...
        if isinstance(input_data, str):
            ...
            # For other types, return a lowercase string representation, 
            # its length, type, and the original input
