def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # For strings, split, sort, remove duplicates, join, and count the number of items
            ...
            # For collections, sort elements, remove duplicates, join, and count the number of items
            sorted_items = sorted(set(str(item).lower() for item in result))
            item_count = len(sorted_items)
            return f"{','.join(sorted_items)}, item count: {item_count}"
        elif isinstance(result, dict):
            # For dictionaries, sort keys, remove duplicates, join, and count the number of items
            sorted_keys = sorted(set(str(key).lower() for key in result.keys()))
            key_count = len(sorted_keys)
            return f"{','.join(sorted_keys)}, key count: {key_count}"
        else:
        ...
        if isinstance(input_data, str):
            ...
            # If it's a dictionary, sort its keys, remove duplicates, join them, and count items
