def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # the original result, and a mathematical operation
            ...
            return f"{sorted_digits}, sum: {sum_of_digits}, sqrt: {square_root:.2f}, original: {result}"
            ...
            avg_length = sum(len(str(item)) for item in result) / len(result) if result else 0
        ...
        if isinstance(input_data, str):
            ...
            # Split the input, sort it, remove duplicates, join it back together, 
            # and count unique words
            ...
            # and calculate average length
            sorted_items = sorted(set(str(item).lower() for item in input_data))
            avg_length = sum(len(str(item)) for item in input_data) / len(input_data) if input_data else 0
            return f"{','.join(sorted_items)}, average length: {avg_length:.2f}"
        else:
            # For other types, return a lowercase string representation, its length, 
            # its type, and the original input
