def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            # the original result, and its square root (if positive)
            sorted_digits = ''.join(sorted(str(abs(result)).lower()))
            sum_of_digits = sum(int(digit) for digit in str(abs(result)) if digit.isdigit())
            output = f"{sorted_digits}, sum: {sum_of_digits}, original: {result}"
            if result > 0:
                output += f", sqrt: {result**0.5:.2f}"
            return output
            ...
            # For functions, return its name and a sample execution
            try:
                sample_result = result()
                return f"function: {result.__name__}, sample result: {sample_result}"
            except:
                return f"function: {result.__name__}, unable to execute"
        else:
        ...
        if isinstance(input_data, str):
            ...
            # For functions, return its name
            return f"function: {input_data.__name__}"
        else:
