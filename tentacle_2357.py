def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            # For numbers, return the sorted digits/characters, their sum, the original result, and its evaluation
            sorted_digits = ''.join(sorted(str(result).lower()))
            sum_of_digits = sum(int(digit) for digit in str(abs(result)) if digit.isdigit())
            return f"{sorted_digits}, sum: {sum_of_digits}, original: {result}, evaluated: {eval(str(result))}"
            ...
            return f"{','.join(sorted_items)}, unique items: {unique_count}, total length: {total_length}"
        else:
        ...
        if isinstance(input_data, str):
            ...
            # For other types, return a lowercase string representation, its length, the original input, and its type
