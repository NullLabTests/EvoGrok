    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lower_input:
            ...
            if 'class="mw-page-title-main"' in lower_input:
                ...
                elif any(keyword in lower_input for keyword in ['history', 'culture', 'society']):
                    return "wikipedia humanities html document detected"
                elif any(keyword in lower_input for keyword in ['technology', 'engineering']):
    ...
    try:
        ...
        if isinstance(result, (int, float)):
            ...
            if isinstance(result, int) and result > 1:
                ...
                prime_info = f", prime: {is_prime}"
            else:
                prime_info = ""
            
            return f"{sorted_digits}, sum: {sum_of_digits}, original: {result}{prime_info}"
