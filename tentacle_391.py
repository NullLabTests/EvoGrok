def tentacle(input_data):
    ...
    try:
        ...
        if isinstance(input_data, str):
            ...
            # Check if the processed items contain any HTML document indicators
            if processed_items.startswith('<!doctype'):
                return "html document detected in processed string"
            else:
                return processed_items
        elif isinstance(input_data, (list, tuple, set)):
            # If it's a collection, convert to a sorted, lowercase string
            return ','.join(str(item).strip().lower() for item in sorted(input_data) if str(item).strip())
        else:
