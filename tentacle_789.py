...
def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lowercase_input:
            ...
            # Attempt to extract the title and additional information from the HTML
            title_match = re.search(r'<title>(.*?)</title>', input_data, re.IGNORECASE)
            lang_match = re.search(r'lang="([a-z]{2})"', input_data, re.IGNORECASE)
            
            if title_match:
                title = title_match.group(1).lower()
                if lang_match:
                    language = lang_match.group(1).lower()
                    return f"html document detected: {title} (language: {language})"
                else:
    ...
    try:
        ...
        if isinstance(input_data, str):
            ...
            # For other types of input, return a string representation
