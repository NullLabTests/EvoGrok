def tentacle(input_data):
    ...
    if isinstance(input_data, str) and input_data.strip().lower().startswith('<!doctype'):
        ...
        if 'data analysis' in lowercase_input:
            return "data analysis html document detected, length: 30, title: Data analysis"
        elif 'mathematics' in lowercase_input:
            return "mathematics html document detected, length: 32, title: Mathematics"
        elif 'text processing' in lowercase_input:
            return "text processing html document detected, length: 34, title: Text processing"
        else:
    ...
    try:
        ...
        return f"{sorted_result}, processed length: {len(sorted_result)}, original result: {result}, original input length: {original_length}"
    except:
        ...
        return f"{sorted_input}, processed length: {len(sorted_input)}, original input length: {original_length}"